"""
Hybrid Vector Database Service for Outage Analysis
...
class HybridOutageVectorDB:
    ...
    def __init__(self, db_path: str = "./chroma_db"):
        ...
        try:
            ...
            logger.error(f"❌ Hybrid Vector DB initialization failed: {str(e)}")
            raise
    
    def load_outage_data(self, df: pd.DataFrame, force_reload: bool = False) -> Dict:
        """Load raw outage data (maintains compatibility with existing code)"""
        try:
            cache_dir = "./cache"
...
WEATHER CONTEXT: {outage.get('weather_conditions', 'Weather data suggests conditions were not severe enough for outages.')}"""
    ...
    def query_outages(self, query_text: str, n_results: int = 10) -> Dict:
        """Query raw outages (maintains compatibility)"""
        try:
            results = self.raw_collection.query(
                query_texts=[query_text],
                n_results=n_results
            )
            return results
        except Exception as e:
            logger.error(f"❌ Error querying raw outages: {str(e)}")
            return {"error": str(e)}
    
    def get_collection_stats(self) -> Dict:
        """Get statistics about both collections"""
        try:
            raw_count = self.raw_collection.count()
            analysis_count = self.analysis_collection.count()
            
            return {
                "raw_outages": raw_count,
                "analysis_results": analysis_count,
                "total_documents": raw_count + analysis_count
            }
        except Exception as e:
